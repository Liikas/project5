...
def get_mask_card_number(card_number: Union[int, str]) -> str:
    ...
    if len(card_number_str) != 16:
        raise ValueError("Enter 16-digit card number")
    elif not card_number_str.isdigit():
        raise TypeError("The card number must be digits only")
    else:
...
def get_mask_account(account_info: Union[str, int]) -> str:
    ...
    if len(account_info_str) != 20:
        raise ValueError("Enter 20-digit account number")
    elif not account_info_str.isdigit():
        raise TypeError("The account number must be digits only")
    else:
